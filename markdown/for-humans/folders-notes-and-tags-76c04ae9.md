...
Notes store the knowledge people and agents reuse. A note can be created from:
...
Use tags when a note belongs to more than one category. For example, a note can live in the `Client Alpha` folder while also being tagged `invoice`, `urgent`, and `legal`.

Tags are also useful when publishing. You can classify notes before export, then publish only the notes that match the tags you choose.
...
Stars are private per user. Starring a note does not make it starred for everyone else. Use stars for notes you personally need to revisit.
